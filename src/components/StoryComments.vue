  ...
  -->
    <div class="card comments" style="margin-top:25px;">
        ...
        <div class="card-content">
            ...
            <p class="content">
                Please always remain civil, courteous and constructive. No spam or self promotion here.
            </p>
            <!-- ADD COMMENT FORM -->
            <form @submit.prevent="postComment" v-if="isAuthenticated">
                <div>
                    ...
                    <div>
                        <p v-if="comment.length <= 5000" class="content is-small is-marginless pb-05">
                            Have a comment about this story? Got a question? Post it.
                        </p>
                        <p v-else class="content isError is-small is-marginless pb-05">Max length 5000 Characters</p>
                    </div>
                    <div class="control">
                ...
                <div class="control mt10p20">
                    <button class="button is-primary" :disabled="comment.trim().length === 0 || comment.length > 5000">
                        Add comment
                    </button>
                </div>
            </form>
            ...
            <div v-for="(comment, idx) in comments" :key="idx">
                ...
                <div class="comment-info">
                    <a @click.prevent="gotoUserProfile(comment.from_username)">
                        {{ comment.from_user_display_name || comment.from_username }}
                    </a>
                    - {{ elapsed(comment.date_created) }}
                    <span v-if="isMyComment(comment.from_user_key)">
                        -
                ...
                <div class="comment-edit" v-if="comment_edit_idx === idx">
                    ...
                    <button class="button is-light is-small mt10p20 ml10" @click.prevent="cancelUpdateComment(idx)">
                        Cancel
                    </button>
                </div>
                <!-- -->
    ...
    methods: {
        async postComment() {
            try {
                ...
                const response = await axiosBase.post(`/stories/${this.story_key}/comments`, {
        ...
        async getComments() {
            try {
                ...
                const response = await axiosBase.get(`/stories/${this.story_key}/comments`);
        ...
        isMyComment: function(comment_from_user_key) {
            return this.isAuthenticated && comment_from_user_key === this.authenticatedUser._key;
        },
        ...
        updateComment: async function(event, comment_key, idx) {
            try {
                ...
                const rez = await axiosBase.put(`/stories/${this.story_key}/comments/${comment_key}`, {
        ...
        deleteComment: async function(comment_key, idx) {
            try {
                ...
                // console.log(comment_key);
                // console.log(idx);
                await axiosBase.delete(`/stories/${this.story_key}/comments/${comment_key}`);
